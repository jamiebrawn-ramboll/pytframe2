...
class VersionControl(Tool):
    ...
    def get_branches(self) -> list[str]:
        return self.parse_git_branches(
            self.git_subprocess("branch", "-a", self.workdir).stdout
        )

    def get_active_branch(self) -> str:
        return self.git_subprocess(
            "branch", "--show-current", self.workdir
        ).stdout.strip()
